                 ...
                 RandomIter last)
{
    ...
    while (last - first > 3)
    {
        auto cut = dwt_stl::unchecked_partition(first, last, dwt_stl::median(*first, *(first + (last - first) / 2), *(last - 1)));
                 ...
                 RandomIter last, Compared comp)
{
    ...
    while (last - first > 3)
    {
        auto cut = dwt_stl::unchecked_partition(first, last, dwt_stl::median(*first, *(first + (last - first) / 2), *(last - 1)), comp);
        if (cut <= nth)   // 如果 nth 位于右段
            first = cut;  // 对右段进行分割
        else
            last = cut;  // 对左段进行分割
