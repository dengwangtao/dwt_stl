...
void arr_sum(s32 i)
{ for_each_sum += i; }
bool cmp(const s32& a, const s32& b)
{ return b < a; }
s32 unary_op(const s32& x)
{ return x + 1; }
s32 binary_op(const s32& x, const s32& y)
{ return x + y; }
...
TEST(next_permutation_test)
{
    ...
    while (std::next_permutation(arr1, arr1 + 4))
        n1++;
    while (dwt_stl::next_permutation(arr1, arr1 + 4))
        n2++;
    EXPECT_EQ(n1, n2);
    for (; n1 > 0; n1--)
    {
...
TEST(prev_permutation_test)
{
    ...
    while (std::prev_permutation(arr1, arr1 + 4))
        n1++;
    while (dwt_stl::prev_permutation(arr1, arr1 + 4))
        n2++;
    EXPECT_EQ(n1, n2);
    for (; n1 > 0; n1--)
    {
