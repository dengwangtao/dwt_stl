...
using color_return_t = typename std::enable_if<
        std::is_same<T, redbud::io::format>::value || std::is_same<T, redbud::io::fg>::value || std::is_same<T, redbud::io::bg>::value || std::is_same<T, redbud::io::hfg>::value || std::is_same<T, redbud::io::hbg>::value,
        std::ostream&>::type;
...
static constexpr WORD default_state = (FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_RED);
...
{
    state &= 0xFF0F;
    state |= (static_cast<WORD>(0x80) | get_rgb(static_cast<WORD>(color) - 100) << 4);
}
...
{
    return (os.iword(details::get_iword()) || (details::is_modifiable() && details::is_terminal(os.rdbuf())))
